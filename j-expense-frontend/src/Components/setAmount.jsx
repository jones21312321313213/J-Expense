...
const styles = {
	...
	modal: {
		width: 340, maxWidth: '92%', background: '#fff', borderRadius: 12, boxShadow: '0 8px 40px rgba(0,0,0,0.25)', overflow: 'hidden'
	},
	header: { padding: '12px 16px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
...
function formatDisplayValue(v) {
	...
	// Keep literal typed decimals when user is in the process of entering
	if (v.includes('.')) return v
	return n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
}

const SetAmount = ({ initialValue = 0, onConfirm = () => {}, onClose = () => {}, title = 'Set Amount' }) => {
	const [value, setValue] = useState('')

	useEffect(() => {
		if (initialValue !== undefined && initialValue !== null) {
			if (typeof initialValue === 'number') setValue(initialValue.toString())
	...
	return (
		<div style={styles.overlay} onMouseDown={onClose}>
			<div style={styles.modal} onMouseDown={(e) => e.stopPropagation()}>
				...
				<div style={styles.display}>{formatDisplayValue(value)}</div>
